...
const Main = () => {
  ...
  return (
    <div className="w-full h-[550px] text-white">
      ...
      <div className="absolute w-full top-[50%] p-4 md:pd-8">
        <h1 className="text-3xl md:text-5xl font-bold">
          {randomPopularMovie?.title}
        </h1>
        <div>
          <button className="border text-white py-2 px-4 rounded-3xl border-blue-600 bg-blue-600 shadow-lg">
            Watch now
          </button>
          <button className="border text-white py-2 px-4 rounded-3xl ml-4">
            Watch trailer
          </button>
          <p className="text-gray-400 text-sm">
            Released: {randomPopularMovie?.release_date}
          </p>
          <p>{randomPopularMovie?.overview}</p>
        </div>
      </div>
    </div>
  );
